...
describe("EscrowService Testing", function () {
    ...
    it("Testing senderSendPayment()", async function () {    
        ...
        await expect( await escrowService.connect(_sender).SendPayment({value: 11})).to.changeEtherBalances([_sender,escrowService], [-11,10]);
    ...
    it("Testing senderDenyDeliver()", async function () {    
        ...
        await expect( await escrowService.connect(deployer).AgentTransfer()).to.changeEtherBalances([_sender,escrowService], [10,-10]);
...
describe("EscrowService Testing 2", function () {
    ...
    it("Testing SellerClaimPayment()", async function () {    
        ...
        await expect ( escrowService.connect(_receiver).ClaimPayment()).to.be.revertedWith('Cannot claim Payment without submission!');
     ...
     it("Testing senderConfirmDeliver()", async function () {    
        ...
        expect (await escrowService.connect(_sender).status()).to.equal(5);

        await expect( await escrowService.connect(deployer).AgentTransfer()).to.changeEtherBalances([_receiver,escrowService], [10,-10]);
